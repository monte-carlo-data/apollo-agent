...
_CUSTOM_BI_CONNECTORS_BASE_PATH = "/opt/custom-bi-connectors"

# Module-level cache: {connection_type: connector_dir_path}
_custom_bi_connector_registry: Optional[Dict[str, str]] = None

# Module-level cache: {module_name: loaded module}
_loaded_modules: Dict[str, types.ModuleType] = {}


def _discover_custom_bi_connectors() -> Dict[str, str]:
    """
    Scan the custom BI connectors directory and build a mapping of
    connection_type -> directory path by reading each manifest.json.
    """
    registry: Dict[str, str] = {}
    base_path = _CUSTOM_BI_CONNECTORS_BASE_PATH

    if not os.path.isdir(base_path):
        logger.info("Custom BI connectors directory not found: %s", base_path)
        return registry
    ...
    for name in sorted(os.listdir(base_path)):
        ...
        if not os.path.isfile(manifest_path):
            logger.warning("Skipping custom BI connector %s: no manifest.json", name)
            continue
        ...
        try:
            ...
            if not connection_type:
                logger.warning(
                    "Skipping custom BI connector %s: no connection_type in manifest",
                    name,
                )
                continue
            registry[connection_type] = connector_dir
            logger.info(
                "Discovered custom BI connector: %s -> %s",
                connection_type,
                connector_dir,
            )
        except Exception:
            logger.exception("Failed to read manifest for custom BI connector %s", name)

    return registry


def get_custom_bi_connector_registry() -> Dict[str, str]:
    """
    Return the cached custom BI connector registry, discovering on first access.
    Contents are baked into the Docker image so they never change at runtime.
    """
    global _custom_bi_connector_registry
    if _custom_bi_connector_registry is None:
        _custom_bi_connector_registry = _discover_custom_bi_connectors()
    return _custom_bi_connector_registry
...
def load_connector_module(connector_dir: str) -> types.ModuleType:
    ...
    module_name = f"custom_bi_connector_{dir_name}"
