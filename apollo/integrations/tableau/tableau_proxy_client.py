...
class TableauProxyClient(BaseProxyClient):
    ...
    def __init__(self, credentials: Optional[Dict], **kwargs: Any):  # noqa
        ...
        if "connect_args" in credentials:
            # CTP path: JWT pre-generated by pipeline, stored in connect_args["token"]
            creds = credentials["connect_args"]
            self._token = creds["token"]
            self._user_name = self._client_id = self._secret_id = self._secret_value = (
                None
            )
            self._token_expiration_seconds = None
            self._site_name = creds.get("site_name", "")
            server_name = creds["server_name"]
            self.verify_ssl = creds.get("verify_ssl", True)
        else:
            # Legacy flat path: generate JWT on each sign-in
            self._token = None
    ...
    def _sign_in(self, expiration_seconds: int):
        token = self._token or generate_jwt(
