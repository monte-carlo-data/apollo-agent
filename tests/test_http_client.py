...
def _build_mule_jar(
    ...
    """Construct an in-memory Mule application JAR for ``extract_mulesoft_sources`` tests.
...
class TestExtractMulesoftSources(TestCase):
    """Tests for HttpProxyClient.extract_mulesoft_sources — streams a Mule JAR
    to a transient tempfile, extracts the listed config XMLs + ``properties/``
    entries, repackages into an in-memory deflate zip, and returns it inline.
    No agent-storage write happens anywhere on this path.
    """
    ...
    def _no_storage_should_be_called(self):
        """Patch ``get_storage_client`` and assert it's never invoked during
        the test body. ``extract_mulesoft_sources`` is meant to be
        storage-free; a regression that accidentally re-introduced an upload
    ...
    def test_missing_config_entry_skipped_op_still_returns_ok(self, mock_get):
        ...
        client = HttpProxyClient(credentials={"connect_args": {}})
        with patch(
            "apollo.integrations.http.http_proxy_client._logger"
        ) as mock_logger:
            with self._no_storage_should_be_called():
                result = client.extract_mulesoft_sources("https://s3.example/app.jar")
