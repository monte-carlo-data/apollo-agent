...
from apollo.integrations.ccp.transforms.load_private_key import LoadPrivateKeyTransform
...
class TestLoadPrivateKeyTransform(TestCase):
    ...
    def test_wrong_passphrase_raises_ccp_error(self):
    ...
    def test_invalid_pem_raises_ccp_error(self):
...
from apollo.integrations.ccp.transforms.oauth import OAuthTransform
...
class TestOAuthTransform(TestCase):
    ...
    @patch("apollo.integrations.ccp.transforms.oauth.requests.post")
    def test_http_error_raises_ccp_error(self, mock_post):
...
from apollo.integrations.ccp.transforms.resolve_presto_auth import (
...
from apollo.integrations.ccp.transforms.write_ini_file import WriteIniFileTransform
...
from apollo.integrations.ccp.transforms.generate_jwt import GenerateJwtTransform
...
from apollo.integrations.ccp.transforms.resolve_msal_token import (
    ResolveMsalTokenTransform,
)
...
class TestResolveMsalTokenTransform(TestCase):
    @patch(
        "apollo.integrations.ccp.transforms.resolve_msal_token.msal.ConfidentialClientApplication"
    )
    ...
    @patch(
        "apollo.integrations.ccp.transforms.resolve_msal_token.msal.PublicClientApplication"
    )
    ...
    def test_msal_error_raises_ccp_error(self, mock_app_cls):
        mock_app = MagicMock()
    ...
    def test_empty_response_raises_ccp_error(self, mock_app_cls):
        mock_app = MagicMock()
...
from apollo.integrations.ccp.transforms.resolve_databricks_oauth import (
    ResolveDatabricksOauthTransform,
)
